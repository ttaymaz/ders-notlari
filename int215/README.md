# İNT215 — Görsel Programlama I

**İnternet ve Ağ Teknolojileri Programı** · Sinanpaşa Meslek Yüksekokulu
Öğr. Gör. Turgay Taymaz · Kısa adres: `tymz.org/int215`
Haftalık 2 saat · 2 AKTS

Bu sayfa dersin kalıcı adresidir. Sınav takvimi ve hafta planı her dönem için
ayrı tutulur; aşağıdan ilgili döneme geçin.

## Dönemler

| Dönem | Durum | Sayfa |
| ----- | ----- | ----- |
| 2026–2027 Güz | **Aktif** | [takvim ve plan](2026-2027-guz/) |

## İçerik

Ders içeriği [`konular/`](../konular/) altında ders kodundan bağımsız tutulur.
Aynı içerik birden fazla derste kullanılır; hangi konunun hangi haftada
işlendiği her dönemin kendi sayfasında yazar.

Bu dönem ders, görsel arayüzün dayandığı nesne tabanlı programlama temelini
kurar: Windows Forms'un tamamı sınıflar, nesneler ve olaylar üzerine kuruludur.
Görsel tasarım bahar döneminde bu temelin üzerine gelir.

## Öncesi

**Algoritma ve Programlama** (birinci sınıf güz dönemi). Bu ders onun devamıdır:
orada üst düzey ifadelerle yazdığımız kodu burada sınıflarla tasarlıyoruz.

## Devamı

**SD222 — Görsel Programlama II** (bahar dönemi). Windows Forms ve SQL Server
ile adım adım bir otomasyon yazılımı geliştirilir; bu dönemde tasarlanan
sınıflar o projenin temelidir.
